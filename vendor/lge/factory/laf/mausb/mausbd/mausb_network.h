...
#ifndef __MAUSB_NETWORK_H
#define __MAUSB_NETWORK_H

#include <sys/types.h>
#include <stdint.h>

#define MAUSB_VERSION 0x00000106
#define MAUSB_PORT 3240
#define MAUSB_PORT_STRING "3240"
...
#define PACK_OP_COMMON(pack, op_common)  do {\
    mausb_net_pack_uint16_t(pack, &(op_common)->version);\
    mausb_net_pack_uint16_t(pack, &(op_common)->code   );\
    mausb_net_pack_uint32_t(pack, &(op_common)->status );\
} while (0)
...
#define PACK_OP_IMPORT_REPLY(pack, reply)  do {\
    mausb_net_pack_usb_device(pack, &(reply)->udev);\
} while (0)
...
#define PACK_OP_EXPORT_REQUEST(pack, request)  do {\
    mausb_net_pack_usb_device(pack, &(request)->udev);\
} while (0)
...
#define PACK_OP_DEVLIST_REPLY(pack, reply)  do {\
    mausb_net_pack_uint32_t(pack, &(reply)->ndev);\
} while (0)
...
void mausb_net_pack_usb_interface(int pack, struct mausb_usb_interface *uinf);
...
int mausb_net_tcp_connect(char *hostname, char *port);

#endif /* __MAUSB_NETWORK_H */
