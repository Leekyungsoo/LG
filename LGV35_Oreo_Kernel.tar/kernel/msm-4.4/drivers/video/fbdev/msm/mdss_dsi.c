...
#if defined(CONFIG_LGE_DISPLAY_BRIGHTNESS_DIMMING)
	if (!ctrl_pdata->lge_ctrl_pdata->bc_reg_backup_flag) {
		lge_bc_dim_reg_backup(ctrl_pdata);
...
#if defined(CONFIG_LGE_DISPLAY_VR_MODE)
	if (!ctrl_pdata->lge_ctrl_pdata->vr_reg_backup) {
		ret = lge_vr_low_persist_reg_backup(ctrl_pdata);

		if (ret < 0)
			pr_err("[Display] %s: fail to backup VR Persist reg.\n", __func__);
		else
			ctrl_pdata->lge_ctrl_pdata->vr_reg_backup = true;
	}
#endif
